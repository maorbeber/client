...
function HomePageParent({ navigation }) {
  ...
  React.useEffect(() => {
    ...
    setCurrentBabySitterDisplay([
        { name: "מאור בבר", location: "ישראל", phoneNumber: "054-9542812", description:"שלום, שמי מאור ואני מסתדר מעולה עם ילדים" },
        { name: "חן ביטון", location: "ישראל", phoneNumber: "054-9542812", description:"היי אני חן ואני מבשלת מעולה" },
    ]);
  }, []);
        ...
        >
          {users.length > 0 ? (
            currentBabySitterDisplay.map((user, index) => (
              <TouchableOpacity
                onPress={() => navigation.navigate("BabySitterInfo", { user })}
              >
                <TouchableRipple style={styles.rowButton} key={index}>
                  <View>
                    ...
                    >
                      <View style={styles.phoneNumberColumn}>
